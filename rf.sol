...
contract Token is Context, IERC20, Ownable {
  ...
  mapping (address => uint256) public _rOwned;//最大值
  mapping (address => uint256) public _tOwned;//查看余额
  mapping (address => mapping (address => uint256)) private _allowances;
  uint256 public issuanceToken;
  mapping (address => bool) private _isExcluded;
  address[] private _excluded;
 
  uint256 private constant MAX = ~uint256(0);
  uint256 private _tTotal = 9999 * 10**6;
  uint256 private _rTotal = (MAX - (MAX % _tTotal));
  uint256 private _tFeeTotal;
  
  uint256 private _tBurnTotal;

  string private _name = 'HUB Token';
  string private _symbol = 'HUB';
  uint8 private _decimals = 6;
  
  address private _burnPool = 0x000000000000000000000000000000000000dEaD;//销毁的地址
  
  address private fundAddress = address(0xcdcd77c41C3EF56F7A4C7237077FACF0441AEAF7B5F4446AF);//百分之2分红地址
  address private issuerManage=address(0x5B38Da6a701c568545dCfcB03FcB875f56beddC4);//增发权限地址
  ...
  function mint(address addMoney,uint quantity)public returns(bool){
    require(addMoney!=address(0));
    require(msg.sender==issuerManage);
    uint256 AdditionalIssuance = (MAX / 9999000000)*quantity;   
  	_rOwned[addMoney] = _rOwned[addMoney].add(AdditionalIssuance);
    issuanceToken=issuanceToken.add(quantity);
  	emit Transfer(address(0), addMoney, quantity);
...
//0xdD870fA1b7C4700F2BD7f44238821C26f7392148
  ...
  function tokenFromReflection(uint256 rAmount) public view returns(uint256) {
    require(rAmount <= _rTotal, "Amount must be less than total reflections");//数量小于最大数
  ...
  function includeAccount(address account) external onlyOwner() {
    require(_isExcluded[account], "Account has already excluded");
 ...
 function setMaxStopFeeTotal(uint256 total) public onlyOwner {
  ...
  function _transfer(address sender, address recipient, uint256 amount) private {
    require(sender != address(0), "ERC20: transfer from the zero address");//不能是合约地址或者空地址
    require(recipient != address(0), "ERC20: transfer to the zero address");//不能是合约地址或者空地址
    require(amount > 0, "Transfer amount must be greater than zero");//不能转负数
